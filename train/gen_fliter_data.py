...
ensemble_dl_modules = []
for file_name_ in os.listdir('../checkpoint/macbert2/'):
    if file_name_.startswith('.'):
        continue
        
    ensemble_dl_module = Bert(config=bert_config)

    ensemble_dl_module.load_state_dict(torch.load('../checkpoint/macbert2/' + file_name_))
...
tm_predictor_instance = TMPredictor(ensemble_dl_modules, tokenizer, tm_train_dataset.cat2id)

from tqdm import tqdm

submit_result = []

with codecs.open('../data/source_datasets/train.jsonl', mode='r', encoding='utf8') as f:
    reader = f.readlines(f)    
...
with open('../data/source_datasets/fliter_train_result2.txt', 'w') as output_data:
