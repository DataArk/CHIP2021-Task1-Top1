...
ensemble_dl_modules = []
for file_name_ in os.listdir('../checkpoint/dialog_chinese-macbert/'):
    if file_name_.startswith('.'):
        continue
        
    ensemble_dl_module = Bert(config=bert_config)

    ensemble_dl_module.load_state_dict(torch.load('../checkpoint/dialog_chinese-macbert/' + file_name_))
...
with open('./dialog_chinese-macbert.txt', 'w') as output_data:
